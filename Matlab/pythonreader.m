...
% Units and definitions
    startframe = find(stepRef,1,'first');
    startframe = startframe - 5;
    %ktangent2 = ktangent2 - (startframe)/1000;
    time = ((time-time(startframe))/1000); % So time will start at 0 [s]
    TX = TX-TX(startframe); % So position starts at 0 [m]
    TY = TY-TY(startframe); % So position starts at 0 [m]
    TXY = sqrt((TX.^2)+(TY.^2));
    RZ = unwrap(RZ);
    RZ = RZ-RZ(startframe);
    if setTimeZero == 1
        n = 1;
        for i = startframe : length(time)
            time2(n) = time(i);
            TX2(n) = TX(i);
            TY2(n) = TY(i);
            TZ2(n) = TZ(i);
        ...
        RZ = RZ2;
        startframe = 1;
        clear time2 TX2 TY2 TZ2 TXY2 RZ2
    end
    ...
    figure('Name','Steprespons for fremadgående bevægelse'); 
        %subplot(2,1,1);
            ...
            ylim([min(RZ) max(RZ)]);
                ...
                hold on;
                plot(time-0.0637125,tangRZ);
                scatter(time(ktangent2)-0.0637125,RZ(ktangent2));
                yline(0,'--');
                hold off;
            grid on;
%         subplot(2,1,2);
